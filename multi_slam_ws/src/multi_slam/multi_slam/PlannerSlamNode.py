...
#         self.declare_parameter('goal_persistence_time', 10.0)
...
#         goal_persistence_time = self.get_parameter('goal_persistence_time').value
...
#                 goal_persistence_time=goal_persistence_time,
...
#         self.gradient_map_pub = self.create_publisher(OccupancyGrid, "/gradient_map", 10)
...
#                 # Get current time for goal persistence
#                 current_time = self.get_clock().now().seconds_nanoseconds()[0]
                
#                 # Debug current goal and position
#                 if self.goal_point is not None:
#                     dist_to_goal = np.linalg.norm(self.position[:2] - self.goal_point)
#                     self.get_logger().info(f"Current position: [{self.position[0]:.2f}, {self.position[1]:.2f}], Goal: [{self.goal_point[0]:.2f}, {self.goal_point[1]:.2f}], Distance: {dist_to_goal:.2f}, Threshold: {self.planner.goal_reached_threshold:.2f}")
#                 else:
#                     self.get_logger().info(f"No current goal point. Position: [{self.position[0]:.2f}, {self.position[1]:.2f}]")
                
#                 control_input, new_goal_point, self.current_path = self.planner.plan_and_control(
#                     self.dt, current_time
#                 )
                
#                 # Debug new goal point and path
#                 if new_goal_point is not None:
#                     self.get_logger().info(f"New goal point from planner: [{new_goal_point[0]:.2f}, {new_goal_point[1]:.2f}]")
#                 else:
#                     self.get_logger().info("Planner returned None for goal point!")
                    
#                 if not self.current_path:
#                     self.get_logger().info("Planner returned empty path")
                
#                 # Update goal point only if it changed
#                 if new_goal_point is not None and (self.goal_point is None or not np.array_equal(new_goal_point, self.goal_point)):
#                     # Add previous goal point to history
#                     if self.goal_point is not None:
...
#                     self.get_logger().info(f"New goal point: ({new_goal_point[0]:.2f}, {new_goal_point[1]:.2f})")
#                 elif new_goal_point is not None:
#                     self.get_logger().info("Goal point unchanged.")
...
#         """Visualize the SLAM state including particles and map."""
...
#             # Publish entropy and boundary map for debugging
#             self.publish_entropy_map()
#             self.publish_boundary_map()
            
#             # 엔트로피 그래디언트 맵 발행
#             self.publish_gradient_map()
            
#             # Publish goal history
#             self.publish_goal_history()
...
#             p.x = float(point[0])
#             p.y = float(point[1])
#             p.z = 0.05  # 바닥보다 약간 위에 표시
#             marker.points.append(p)
        
#         # Set scale
#         marker.scale.x = 0.08  # 라인 두께 증가
        
#         # Set color - 밝은 녹색으로 변경
#         marker.color.r = 0.2
#         marker.color.g = 1.0
#         marker.color.b = 0.2
#         marker.color.a = 0.8
...
#         waypoints_marker.color.g = 0.6
#         waypoints_marker.color.b = 1.0
#         waypoints_marker.color.a = 0.7
        
#         self.planned_path_pub.publish(waypoints_marker)
        
#         # 3. 현재 목표 웨이포인트 표시 (현재 인덱스)
#         if hasattr(self.planner, 'current_path_index') and self.planner.current_path_index < len(self.current_path):
#             current_wp = self.current_path[self.planner.current_path_index]
            
#             current_marker = Marker()
#             current_marker.header.frame_id = "map"
#             current_marker.header.stamp = self.get_clock().now().to_msg()
#             current_marker.ns = "current_waypoint"
#             current_marker.id = 2
#             current_marker.type = Marker.SPHERE
#             current_marker.action = Marker.ADD
            
#             # 위치 설정
#             current_marker.pose.position.x = float(current_wp[0])
#             current_marker.pose.position.y = float(current_wp[1])
#             current_marker.pose.position.z = 0.05
...
#             self.planned_path_pub.publish(current_marker)
...
#             # 텍스트 설정
#             text_marker.text = f"WP {self.planner.current_path_index}/{len(self.current_path)-1}"
...
#             self.planned_path_pub.publish(text_marker)
...
#     def publish_gradient_map(self):
#         """Publish entropy gradient map visualization."""
#         if not self.use_planner or self.planner is None:
#             return
...
#         if not hasattr(self.planner, 'entropy_map') or self.planner.entropy_map is None:
#             try:
#                 self.planner.generate_entropy_map()
#             except Exception as e:
#                 self.get_logger().error(f"엔트로피 맵 생성 오류: {e}")
#                 return
                
#         # 엔트로피 맵이 여전히 없으면 실패
#         if not hasattr(self.planner, 'entropy_map') or self.planner.entropy_map is None:
#             self.get_logger().warn("엔트로피 맵 생성 후에도 맵이 없습니다.")
#             return
            
#         # 그래디언트 맵이 없으면 계산
#         if not hasattr(self.planner, 'gradient_magnitude') or self.planner.gradient_magnitude is None:
#             try:
#                 # 엔트로피 그래디언트 계산 - entropy_map을 인수로 전달
#                 gradient_x, gradient_y, gradient_magnitude = self.planner.compute_entropy_gradient(self.planner.entropy_map)
...
#                     # 디버그 메시지 출력
#                     self.get_logger().info("그래디언트 맵 속성이 없음 - 직접 계산합니다.")
                    
#                     # 엔트로피 맵에서 그래디언트 직접 계산
#                     gradient_x = sobel(self.planner.entropy_map, axis=1)
#                     gradient_y = sobel(self.planner.entropy_map, axis=0)
...
#                 else:
#                     gradient_magnitude = self.planner.gradient_magnitude
#             except Exception as e:
#                 self.get_logger().error(f"그래디언트 계산 오류: {e}")
#                 return
#         else:
#             gradient_magnitude = self.planner.gradient_magnitude
        
#         if gradient_magnitude is None:
#             self.get_logger().warn("그래디언트 맵이 여전히 없습니다.")
#             return
            
#         # 시각화를 위해 그래디언트 맵을 0-100 범위로 변환
#         gradient_scaled = (gradient_magnitude * 100).astype(int)
...
#         # 그래디언트 값 설정
#         msg.data = gradient_scaled.flatten().tolist()
        
#         # 발행
#         self.gradient_map_pub.publish(msg)

def publish_entropy_map(self):
        """Publish entropy map visualization."""
        if not hasattr(self.planner, 'entropy_map') or self.planner.entropy_map is None:
            # Try to generate the entropy map if it doesn't exist
            if self.planner is not None and hasattr(self.planner, 'generate_entropy_map'):
                try:
                    self.planner.generate_entropy_map()
                except Exception as e:
                    self.get_logger().error(f"Error generating entropy map: {str(e)}")
                    return
            else:
                return
                
        # Check again if entropy map exists after generation attempt
        if not hasattr(self.planner, 'entropy_map') or self.planner.entropy_map is None:
            return
        ...
        self.entropy_map_pub.publish(msg)# slam_node.py
import rclpy
from rclpy.node import Node
from sensor_msgs.msg import PointCloud2
from geometry_msgs.msg import Vector3, PoseStamped, Point
...
class PlannerSLAMNode(Node):
    ...
    def __init__(self):
        ...
        # Get planner parameters
        self.use_planner = self.get_parameter('use_planner').value
        astar_allow_diagonal = self.get_parameter('astar_allow_diagonal').value
        pd_p_gain = self.get_parameter('pd_p_gain').value
        pd_d_gain = self.get_parameter('pd_d_gain').value
        entropy_weight = self.get_parameter('entropy_weight').value
        beacon_weight = self.get_parameter('beacon_weight').value
        ...
        if self.use_planner:
            self.planner = Planner(
                map_resolution=grid_size,
                astar_allow_diagonal=astar_allow_diagonal,
                pd_p_gain=pd_p_gain,
                pd_d_gain=pd_d_gain,
                entropy_weight=entropy_weight,
                beacon_weight=beacon_weight,
        ...
        # A* visualization publishers (replacing RRT visualization)
        self.astar_path_pub = self.create_publisher(Marker, "/astar_path", 10)
        self.astar_open_set_pub = self.create_publisher(Marker, "/astar_open_set", 10)
        self.astar_closed_set_pub = self.create_publisher(Marker, "/astar_closed_set", 10)
        self.entropy_map_pub = self.create_publisher(OccupancyGrid, "/entropy_map", 10)
        self.boundary_map_pub = self.create_publisher(OccupancyGrid, "/boundary_map", 10)
        self.gradient_map_pub = self.create_publisher(OccupancyGrid, "/gradient_map", 10)
    ...
    # Add these methods to your PlannerSLAMNode class
    ...
    def publish_goal_point(self):
        ...
        # Current time for the text display
        current_time = self.get_clock().now()
        time_str = f"{current_time.seconds_nanoseconds()[0]}"
        
        # 1. Goal point sphere marker (larger and brighter color)
        sphere_marker = Marker()
        sphere_marker.header.frame_id = "map"
        sphere_marker.header.stamp = current_time.to_msg()
        sphere_marker.ns = "goal_point"
        ...
        # 2. Goal point arrow marker (large upward arrow)
        arrow_marker = Marker()
        arrow_marker.header.frame_id = "map"
        arrow_marker.header.stamp = current_time.to_msg()
        arrow_marker.ns = "goal_point_arrow"
        ...
        # 3. Goal point text marker (showing coordinates and time)
        text_marker = Marker()
        text_marker.header.frame_id = "map"
        text_marker.header.stamp = current_time.to_msg()
        text_marker.ns = "goal_point_text"
        ...
        text_marker.pose.position.x = float(self.goal_point[0])
        text_marker.pose.position.y = float(self.goal_point[1])
        text_marker.pose.position.z = 0.7  # Above the sphere
        
        # Text content (coordinates and time)
        text_marker.text = f"Goal: ({self.goal_point[0]:.2f}, {self.goal_point[1]:.2f}) | t={time_str}"
    ...
    def publish_astar_visualization(self):
        """Publish A* visualization data."""
        if not self.use_planner or self.planner is None:
            return
        ...
        # A* open set
        self.publish_astar_open_set()
        
        # A* closed set
        self.publish_astar_closed_set()

    def publish_astar_path(self):
        """Publish the A* path as a line strip."""
        if not hasattr(self.planner, 'astar_path') or not self.planner.astar_path:
            return
        ...
        # Add all path points
        for node in self.planner.astar_path:
            world_point = self.planner.grid_to_world(node[0], node[1])
            p = Point()
            p.x = float(world_point[0])
            p.y = float(world_point[1])
        ...
        # Set color - green for A* path
        marker.color.r = 0.0
        marker.color.g = 1.0
        marker.color.b = 0.0
        marker.color.a = 0.8
        
        self.astar_path_pub.publish(marker)

    def publish_astar_open_set(self):
        """Publish the A* open set as points."""
        if not hasattr(self.planner, 'astar_open_set') or not self.planner.astar_open_set:
            return
            
        marker = Marker()
        marker.header.frame_id = "map"
        marker.header.stamp = self.get_clock().now().to_msg()
        marker.ns = "astar_open_set"
        marker.id = 0
        marker.type = Marker.POINTS
        marker.action = Marker.ADD
        
        # Add all open set nodes
        for node in self.planner.astar_open_set:
            world_point = self.planner.grid_to_world(node[0], node[1])
            p = Point()
            p.x = float(world_point[0])
            p.y = float(world_point[1])
        ...
        # Set color - blue for open set
        marker.color.r = 0.0
        marker.color.g = 0.5
        marker.color.b = 1.0
        marker.color.a = 0.5
        
        self.astar_open_set_pub.publish(marker)

    def publish_astar_closed_set(self):
        """Publish the A* closed set as points."""
        if not hasattr(self.planner, 'astar_closed_set') or not self.planner.astar_closed_set:
            return
            
        marker = Marker()
        marker.header.frame_id = "map"
        marker.header.stamp = self.get_clock().now().to_msg()
        marker.ns = "astar_closed_set"
        marker.id = 0
        marker.type = Marker.POINTS
        marker.action = Marker.ADD
        
        # Add all closed set nodes
        for node in self.planner.astar_closed_set:
            world_point = self.planner.grid_to_world(node[0], node[1])
            p = Point()
            p.x = float(world_point[0])
            p.y = float(world_point[1])
        ...
        # Set scale
        marker.scale.x = 0.08  # Point width
        marker.scale.y = 0.08  # Point height
        
        # Set color - red for closed set
        marker.color.r = 1.0
        marker.color.g = 0.2
        marker.color.b = 0.2
        marker.color.a = 0.5
        
        self.astar_closed_set_pub.publish(marker)
    ...
    def publish_goal_history(self):
        ...
        # Create full history including current goal point
        full_history = self.goal_history.copy()
        if self.goal_point is not None:
            full_history.append(self.goal_point)
            
        # Goal trajectory as LINE_STRIP
        marker = Marker()
        marker.header.frame_id = "map"
        marker.header.stamp = self.get_clock().now().to_msg()
        marker.ns = "goal_history"
        ...
        # Each trajectory point as small spheres
        points_marker = Marker()
        points_marker.header.frame_id = "map"
        points_marker.header.stamp = self.get_clock().now().to_msg()
        points_marker.ns = "goal_history_points"
    ...
    def publish_boundary_map(self):
        """Publish boundary map visualization."""
        if not hasattr(self.planner, 'boundary_map') or self.planner.boundary_map is None:
            # Try to generate the boundary map if it doesn't exist
            if self.planner is not None and hasattr(self.planner, 'generate_boundary_map'):
                try:
                    self.planner.generate_boundary_map()
                except Exception as e:
                    self.get_logger().error(f"Error generating boundary map: {str(e)}")
                    return
            else:
                return
                
        # Check again if boundary map exists after generation attempt
        if not hasattr(self.planner, 'boundary_map') or self.planner.boundary_map is None:
            return
    ...
    def publish_gradient_map(self):
        ...
        # Generate entropy map if needed
        if not hasattr(self.planner, 'entropy_map') or self.planner.entropy_map is None:
            try:
                self.planner.generate_entropy_map()
            except Exception as e:
                self.get_logger().error(f"Error generating entropy map: {e}")
                return
                
        # Check again if entropy map exists after generation attempt
        if not hasattr(self.planner, 'entropy_map') or self.planner.entropy_map is None:
            self.get_logger().warn("Entropy map still None after generation attempt")
            return
            
        # Generate gradient if needed
        if not hasattr(self.planner, 'gradient_magnitude') or self.planner.gradient_magnitude is None:
            try:
                # Compute entropy gradient - passing the entropy map as argument
                _, _, _ = self.planner.compute_entropy_gradient(self.planner.entropy_map)
                
                # Compute manually if still not available
                if not hasattr(self.planner, 'gradient_magnitude') or self.planner.gradient_magnitude is None:
                    from scipy.ndimage import sobel
                    import numpy as np
                    
                    # Debug message
                    self.get_logger().info("Computing gradient manually")
                    
                    # Compute gradient directly from entropy map
                    gradient_x = sobel(self.planner.entropy_map, axis=1)
                    gradient_y = sobel(self.planner.entropy_map, axis=0)
                    ...
                    # Store for reuse
                    self.planner.gradient_magnitude = gradient_magnitude
                
                # Use the gradient magnitude computed by the planner
                gradient_magnitude = self.planner.gradient_magnitude
            except Exception as e:
                self.get_logger().error(f"Error computing gradient: {e}")
                return
        else:
            gradient_magnitude = self.planner.gradient_magnitude
        
        if gradient_magnitude is None:
            self.get_logger().warn("Gradient magnitude still None")
            return
    ...
    def sim_done_cb(self, msg: Bool):
        ...
        if self.use_planner and self.planner is not None:
            ...
            try:
                ...
                self.publish_astar_visualization()
                self.publish_goal_history()
