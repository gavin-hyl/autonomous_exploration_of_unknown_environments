...
# # #     Subscribes to the estimated pose from SLAM, planned control commands,
# # #     and manual control inputs from teleop_keyboard.
# # #     Prioritizes manual control when available for direct operator intervention.
# # #     """
...
# # #         # Manual control subscriber - directly subscribe to control_signal from teleop_keyboard
# # #         # We use a separate callback to distinguish teleop commands from planned ones
# # #         self.manual_control_sub = self.create_subscription(
# # #             Vector3, "/teleop_control", self.manual_control_callback, 10
# # #         )
...
# # #         self.declare_parameter("manual_control_timeout", 0.5)  # shorter timeout for better responsiveness
# # #         self.manual_control_timeout = self.get_parameter("manual_control_timeout").value
...
# # #     def manual_control_callback(self, msg: Vector3):
# # #         """Store the latest manual control input from teleop keyboard."""
# # #         # Only update if the control is non-zero (active command)
# # #         if abs(msg.x) > 0.01 or abs(msg.y) > 0.01:
# # #             self.manual_control_input = np.array([msg.x, msg.y, 0.0])
# # #             self.last_manual_control_time = self.get_clock().now().seconds_nanoseconds()[0]
# # #             self.manual_control_active = True
# # #             self.get_logger().info(f"Received manual control: [{msg.x:.2f}, {msg.y:.2f}]")
# # #         else:
# # #             # Zero command - could be a stop command or just no input
# # #             self.manual_control_input = np.array([0.0, 0.0, 0.0])
# # #             self.last_manual_control_time = self.get_clock().now().seconds_nanoseconds()[0]
# # #             # Keep manual_control_active true for a brief period so robot stops
# # #             # rather than immediately switching back to autonomous mode
# # #             self.get_logger().debug("Received zero manual control command")
...
# # #     def is_control_input_stale(self, input_time, timeout):
# # #         """Check if the provided control input timestamp is too old to use."""
# # #         if input_time == 0.0:
# # #             return True
...
# # #         """Check if manual control is currently active and not stale."""
# # #         if self.manual_control_input is None:
# # #             return False
            
# # #         return not self.is_control_input_stale(
# # #             self.last_manual_control_time, 
# # #             self.manual_control_timeout
# # #         )

# # #     def control_loop(self):
# # #         """
# # #         Generate and publish control command based on control priorities.
        
# # #         Priority order:
# # #         1. Manual control (if recent)
# # #         2. Planned control (if planning is active and control is fresh)
# # #         3. Fallback control (if enabled)
# # #         4. Stop (if no valid control and no fallback)
# # #         """
# # #         # First priority: Check if manual control is active and recent
# # #         if self.is_manual_control_active():
# # #             control_input = self.manual_control_input
# # #             self.get_logger().debug("Using manual control input")
            
# # #         # Second priority: Check if we should use planned control
# # #         elif (self.is_planning_active and 
# # #               self.planned_control_input is not None and 
# # #               not self.is_control_input_stale(self.last_planned_control_time, self.control_timeout)):
            
# # #             control_input = self.planned_control_input
# # #             self.get_logger().debug("Using planned control input")
            
# # #         # Third priority: Use fallback control if enabled
# # #         elif self.use_manual_fallback:
# # #             control_input = self.fallback_control
# # #             self.get_logger().debug("Using fallback control input")
            
# # #         # Last resort: Stop if no valid control and no fallback
# # #         else:
# # #             control_input = np.zeros(3)
# # #             self.get_logger().debug("No valid control input, stopping")
